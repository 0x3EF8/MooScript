...
module.exports = async function appstateHandler(ctx) {
    ...
    const isValidAppState = appStateData.every((state) => requiredKeys.every((key) => Object.prototype.hasOwnProperty.call(state, key)));
    ...
    const isDuplicate = appStateData.some((newCookie) => existingAppStateValues.some((existingCookie) => JSON.stringify(newCookie) === JSON.stringify(existingCookie)));

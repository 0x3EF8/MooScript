...
const vipListPath = path.join(__dirname, '..', 'json', 'userpanel.json');
const vipList = require(vipListPath);

async function handleUnsend(api, event) {
  try {
    const messagesData = {};
    async function sendToAdmins(message) {
      for (const adminUid of config.admin) {
        try {
          await api.sendMessage(message, adminUid);
        } catch (err) {
          console.error("Failed to send message to admin " + adminUid + ": " + err);
        }
      }
    }
    ...
    try {
      /*
      let data = await database.readData('messages');
      if(data) {
        Object.assign(messagesData, JSON.parse(data));
      }
      */
      if (fs.existsSync(path.join(__dirname, 'messagesData.json'))) {
        const data = fs.readFileSync(path.join(__dirname, 'messagesData.json'), 'utf-8');
        if (data) {
          Object.assign(messagesData, JSON.parse(data));
        }
      }
    ...
    if (event.type === 'message' || event.type === 'message_reply') {
      ...
      //database.updateData('messages', JSON.stringify(messagesData)).then((r) => console.log(r)).catch((err) => console.log(err))

      fs.writeFile(path.join(__dirname, 'messagesData.json'), JSON.stringify(messagesData, null, 2), (err) => {
    ...
    try {
      ...
      if (settings && settings[0] && settings[0].antiunsend === true) {
        const isVIP = vipList.userpanel.VIPS.includes(event.senderID);

        if (event.type === 'message_unsend' && !isVIP) {

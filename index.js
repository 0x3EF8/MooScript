    ...
    async (err, data) => {
        ...
        try {
            ...
            if (bar.complete) {
                ...
                console.log(chalk.red(`Failed to load commands: ${bar.total - bar.curr}\n`));
            }
                ...
                try {
                    ...
                    login({ appState: appStateData }, async (err, api) => {
                        ...
                        api.getUserInfo(api.getCurrentUserID(), (err, ret) => {
                            if (err) {
                                handleError(`Failed to get user info. AppState file: ${appState}.`, err, api);
                                return;
                            }
                            ...
                            try {
                                if (event.type === "message" || event.type === "message_reply") {
                                    ...
                                    if (matchingCommand) {
                                        ...
                                        if (!settings[0].sys && !userpanel.userpanel.VIPS.includes(event.senderID)) {
                                            api.sendMessage("⚠️ Oops: \n\nThe system is currently under maintenance. \nNote: Only authorized users may use commands during this state.", event.threadID);
                                            return;
                                        }
                                ...
                                handleError("An error occurred in the listenMqtt function:", error, api);
                            }
                        });
                    });
                } catch (error) {
                    handleError(`Failed to read appstate file: ${appState}.`, error);
                }
            }
        } catch (error) {

...
describe('Login:', function () {
  ...
  process.on(
    'SIGINT',
    () => api && !api.logout() && console.log('Logged out :)')
  );
  var tests = [];
  var stopListening;
  this.timeout(20000);
  ...
  it('should send text message object (user)', function (done) {
    ...
    listen(
      ...
      (msg) =>
        msg.type === 'message' && msg.body === body && msg.isGroup === false
    );
  ...
  it('should send text message object (group)', function (done) {
    ...
    listen(
      ...
      (msg) =>
        msg.type === 'message' && msg.body === body && msg.isGroup === true
    );
  ...
  it('should parse share attachment correctly', function () {
    ...
    assert(
      formatted.attachments[0].target.items[0].call_to_actions.length === 3
    );
    assert(
      formatted.attachments[0].target.items[0].call_to_actions[0].title ===
        'Google'
    );
  });

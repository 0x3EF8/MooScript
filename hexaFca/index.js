...
function buildAPI(globalOptions, html, jar) {
  ...
  // Dynamically load all JavaScript files from the /src/ directory 
  const apiFuncNames = fs.readdirSync(__dirname + '/src')
    .map((file) => file.endsWith('.js') ? file.replace('.js', '') : null)
    .filter(Boolean);  // Remove null values (non-JS files)

  var defaultFuncs = utils.makeDefaults(html, userID, ctx);

  // Load all API functions dynamically in a loop
  apiFuncNames.map(function (v) {
    api[v] = require(__dirname + '/src/' + v)(defaultFuncs, api, ctx);
  });

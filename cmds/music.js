...
async function music(event, api) {
  ...
  if (data.includes('-help')) {
    const usage = "Usage: music [song title]\n\n" +
      "Description: Searches and downloads music from YouTube. You can also include the -lyrics option to retrieve the lyrics along with the music.\n\n" +
      "Example: music Giga Chad Theme song\n" +
      "Example with lyrics: music -lyrics Yesterday by The Beatles\n\n" +
  ...
  api.sendMessage(`🔍 Searching for the music: ${songTitle}...`, event.threadID, event.messageID);
  ...
  async function main() {
    ...
    if (isLyricsIncluded) {
      ...
      if (lyricsData) {
        const lyricsMessage = `Music Title "${lyricsData.s_title}" by ${lyricsData.s_artist}:\n\n${lyricsData.s_lyrics}`;

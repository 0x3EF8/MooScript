const fs = require('fs');
const path = require('path');

function readConfig() {
  const configPath = path.join(__dirname, '..', 'json', 'config.json');
  try {
    return JSON.parse(fs.readFileSync(configPath));
  } catch (error) {
    console.error('Error reading config:', error);
    return null;
  }
}

function isAdmin(userId) {
  const config = readConfig();
  if (config !== null && config.hasOwnProperty('admin')) {
    const adminList = config.admin || [];
    return adminList.includes(userId);
  }
  return false;
}

function AdminCommand(event, api) {
  if (event.body.includes('-help')) {
    const usage = "Usage: admin [-add/-rem] [user ID]\n\n" +
      "Description:\n" +
      "  - admin -add: Adds the specified user to the admin list.\n" +
      "  - admin -rem: Removes the specified user from the admin list.\n\n" +
      "Note: Only admins can use this command.";
    api.sendMessage(usage, event.threadID);
    return Promise.resolve();
  }

  const command = event.body.split(' ')[1];

  if (command === '-add' || command === '-rem') {
    if (!isAdmin(event.senderID)) {
      api.sendMessage("Only admins can use this command.", event.threadID);
      return Promise.resolve();
    }

    if (command === '-add') {
      return addAdmin(event, api);
    } else if (command === '-rem') {
      return remAdmin(event, api);
    }
  } else {
    const config = readConfig();
    if (config !== null && config.hasOwnProperty('admin')) {
      const adminList = config.admin.map(userId => `├─⦿ ${userId}`).join('\n');
      const totalAdmins = config.admin.length;
      const message = `
┌────[ Hexabot Admin Users ]────⦿
│
${adminList}
│
└────[ Total Admin users: ${totalAdmins} ]────⦿
`;
      api.sendMessage(message, event.threadID);
    } else {
      api.sendMessage("An error occurred while reading the admin user list.", event.threadID);
    }
    return Promise.resolve();
  }
}

function addAdmin(event, api) {
  return new Promise((resolve, reject) => {
    const { threadID, messageReply } = event;
    if (!messageReply) return resolve();

    const configPath = path.join(__dirname, '..', 'json', 'config.json');
    const config = readConfig();
    const adminList = config.admin || [];

    const userId = messageReply.senderID;

    api.getUserInfo(parseInt(userId), (error, data) => {
      if (error) {
        console.error(error);
        return reject(error);
      }
      const name = data[userId].name;
      if (adminList.includes(userId)) {
        api.sendMessage(`${name} is already an admin.`, threadID);
        resolve();
      } else {
        adminList.push(userId);
        config.admin = adminList;
        fs.writeFileSync(configPath, JSON.stringify(config, null, 2), "utf8");
        api.sendMessage(`${name} has been successfully added as an admin.`, threadID);
        resolve();
      }
    });
  });
}

function remAdmin(event, api) {
  return new Promise((resolve, reject) => {
    const { threadID, messageReply } = event;
    if (!messageReply) return resolve();

    const configPath = path.join(__dirname, '..', 'json', 'config.json');
    const config = readConfig();
    const adminList = config.admin || [];

    const userId = messageReply.senderID;

    api.getUserInfo(parseInt(userId), (error, data) => {
      if (error) {
        console.error(error);
        return reject(error);
      }

      const name = data[userId].name;

      if (adminList.includes(userId)) {
        const removeIndex = adminList.indexOf(userId);
        adminList.splice(removeIndex, 1);
        config.admin = adminList;
        fs.writeFileSync(configPath, JSON.stringify(config, null, 2), "utf8");
        api.sendMessage(`${name} is no longer an admin.`, threadID);
        resolve();
      } else {
        api.sendMessage(`${name} is not found in the admin list.`, threadID);
        resolve();
      }
    });
  });
}

module.exports = AdminCommand;

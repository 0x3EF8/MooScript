...
async function getStripeInfo(event, api) {
    ...
    try {
        ...
        const amount = responseData.line_item_group?.line_items[0]?.total || responseData.invoice?.total || "Amount information not available";

        const email = responseData.customer?.email || responseData.customer_email || "Email information not available";

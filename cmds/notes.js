...
function loadUserPanel() {
  try {
    const data = fs.readFileSync(userPanelFilePath, 'utf-8');
    return JSON.parse(data);
  } catch (error) {
    return { userpanel: { VIPS: [] } };
  }
}

function isVIP(senderID) {
  const userPanel = loadUserPanel();
  return userPanel.userpanel.VIPS.includes(senderID);
}
